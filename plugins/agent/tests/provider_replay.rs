...
#[test]
fn verifier_rejects_unexpected_target_action_before_decision_record() {
    let host = HostFixture::new();
    // Put an action event at seq=1 before any decision record.
    let action_first = vec![event(1, host.agent, EVENT_TYPE_ACTION, vec![0x80])];
    assert_eq!(
        host.verifier().verify(&action_first, None).unwrap_err(),
        ReplayVerificationError::UnexpectedTargetAction
    );
}

#[test]
fn verifier_reports_decision_record_mismatch_for_wrong_timeline() {
    let host = HostFixture::new();
    let events = accepted_events(&host);
    // Build a verifier for a DIFFERENT timeline; the PDR1's timeline_id won't match.
    let wrong_timeline_verifier = AgentDecisionReplayVerifier::try_new_with_timeline_ancestry(
        vec![TimelineHistorySegment::new(
            TimelineId::from_ulid(fixed_ulid(0x99)),
            Seq::from_u64(32),
        )],
        host.agent,
        host.provenance.clone(),
    ...
    .unwrap();
    assert_eq!(
        wrong_timeline_verifier.verify(&events, None).unwrap_err(),
        ReplayVerificationError::DecisionRecordMismatch
    );
}

#[test]
fn verifier_reports_missing_or_mismatched_action_when_no_action_follows_accepted_decision() {
    let host = HostFixture::new();
    // Only the decision record; the accepted action event is absent.
    let record = host.record(
        0,
        0,
        FixtureResult::Accepted {
    ...
    assert_eq!(
        host.verifier().verify(&record_only, None).unwrap_err(),
        ReplayVerificationError::MissingOrMismatchedAcceptedAction
    );
}

#[test]
fn verifier_reports_missing_checkpoint_when_provided_prefix_does_not_reach_checkpoint() {
    let host = HostFixture::new();
    let events = accepted_events(&host);
    // First get a checkpoint for the full 2-event sequence (last_verified=2).
    let full_checkpoint = host.verifier().verify(&events, None).unwrap();
    // Now verify with only the first event; checkpoint claims last_verified=2 but
    // the slice only reaches seq=1 → MissingCheckpoint.
    assert!(matches!(
        host.verifier()
            .verify(&events[..1], Some(full_checkpoint))
            .unwrap_err(),
