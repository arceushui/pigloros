...
| iai-callgrind | Measures deterministic instruction/cache behaviour under Callgrind rather than the harness's wall-clock `Instant` durations; requires a distinct runtime/tooling and answers a different question | **Do not add.** Consider separately only if the project deliberately wants instruction-count regression policy. |
