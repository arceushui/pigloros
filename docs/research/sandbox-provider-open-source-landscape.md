...
The best initial implementation is therefore a small PiglorOS-owned **systemd Sandbox Provider Adapter**. Use [`zbus`](https://github.com/z-galaxy/zbus) for typed systemd D-Bus calls, [`rtnetlink`](https://github.com/rust-netlink/rtnetlink) plus [`netlink-packet-route`](https://github.com/rust-netlink/netlink-packet-route) for route/link operations, and prototype direct nftables netlink with [`netlink-packet-netfilter`](https://github.com/rust-netlink/netlink-packet-netfilter). Keep the portable provider protocol, admission, transcript, revocation, and evidence logic in PiglorOS.
...
- **Adopt:** zbus for systemd D-Bus; rtnetlink/netlink-packet-route for route netlink.
