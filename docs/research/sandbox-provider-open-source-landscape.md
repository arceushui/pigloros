...
A second hosted run, [33753860005](https://github.com/arceushui/pigloros/actions/runs/33753860005), executed the compiled probe with privilege on the ephemeral GitHub-hosted VM. It created and stopped a transient systemd slice through `zbus_systemd`, created/read/deleted a dummy link through `rtnetlink`, and performed a direct nftables netlink generation read through `netlink-packet-netfilter`; all succeeded without `systemd-run`, `ip`, or `nft`. This establishes the basic typed lifecycle path, but atomic nftables mutation/read-back, namespace-handle ownership, dm-verity, broker-death cleanup, concurrency, and latency targets remain #211 work.
