use super::{
    ...
    inventory_replay_claim, inventory_transitions_preserve_or_weaken, receipt_core_value,
    receipt_from_fields, receipt_value, reference_zero, sort_inventories, state_core_value,
    state_from_fields, state_value, verify_predecessor_chain, BTreeMap, BTreeSet,
...
impl ErasureReceiptV1 {
    ...
    pub fn new(mut input: ErasureReceiptInputV1) -> Result<Self, ErasureErrorV1> {
        ...
        if input.replay_claim != inventory_replay_claim(&input.inventories) {
            return Err(ErasureErrorV1::PolicyConflict);
        }
