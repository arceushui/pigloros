...
fn assert_manifest_read_fault_at_entry(
    invoke: impl FnOnce(
        &mut ErasureCoordinatorStateMachineV1<PublicCoordinatorPort>,
        &ErasureRequestV1,
    ) -> Result<(), ErasureErrorV1>,
) -> Result<(), ErasureErrorV1> {
    let graph = complete_persisted_graph(None)?;
    ...
    assert_eq!(
        invoke(&mut coordinator, &graph.request),
        Err(ErasureErrorV1::TrustSnapshotInvalid)
    );
    Ok(())
}
...
#[test]
fn coordinator_public_missing_state_active_conflict_and_exact_retry_paths(
) -> Result<(), ErasureErrorV1> {
    let target = target(10);
    let request = coordinator_request()?;
    ...
    assert_eq!(
        missing.authorize(reference(240), reference(241)),
        Err(ErasureErrorV1::ProvenanceMissing)
    );
    let correction = correction_for(reference(242), reference(243), reference(244))?;
    assert_eq!(
        missing.submit_corrected(corrected_request(correction.reference())?, correction),
        Err(ErasureErrorV1::ProvenanceMissing)
    );
    ...
    let conflicting_admission = ErasureRetryAdmissionV1::new(ErasureRetryAdmissionInputV1 {
        ...
        source_receipt: admission.source_receipt(),
        unresolved_obligations: admission.unresolved_obligations().to_vec(),
        command_identities: admission.command_identities().to_vec(),
        policy: admission.policy(),
        trust: admission.trust(),
        admitted_position: admission.admitted_position() + 1,
        deadline_position: admission.deadline_position(),
        authorization_provenance: admission.authorization_provenance(),
    })?;
    assert_eq!(
        coordinator.dispatch_attempt(request.reference(), &conflicting_admission),
        Err(ErasureErrorV1::PolicyConflict)
    );
    coordinator.acknowledge(
        request.reference(),
    ...
    coordinator.finalize(request.reference(), input.clone())?;
    assert_eq!(
        coordinator.finalize(
            request.reference(),
            ErasureReceiptInputV1 {
                issue_position: input.issue_position + 1,
                ..input
            },
        ),
        Err(ErasureErrorV1::PolicyConflict)
...
#[test]
fn coordinator_public_requires_persisted_scope_and_lineage_before_extensions(
) -> Result<(), ErasureErrorV1> {
    let target = target(10);
    let request = coordinator_request()?;
    let lineage_rule = reference(170);
    ...
    let resolution =
        ErasureAdministrativeResolutionV1::new(ErasureAdministrativeResolutionInputV1 {
            request: request.reference(),
            affected_digests: vec![reference(171)],
            action: ErasureAdministrativeResolutionActionV1::CloseContainment,
            scope_commitment: scope.reference(),
            policy: reference(5),
            trust: reference(6),
            principal: reference(172),
            authorization_provenance: reference(173),
            reason: reference(174),
            issue_position: 31,
            predecessor_resolution: None,
        })?;
    ...
    assert_eq!(
        submitted.resolve_administratively(request.reference(), &resolution),
        Err(ErasureErrorV1::ProvenanceMissing)
    );

    let mut frozen =
        ErasureCoordinatorStateMachineV1::new(coordinator_port(vec![target], None), COORDINATOR);
    submit_authorize_and_freeze(&mut frozen, &request)?;
    assert_eq!(
        frozen.append_scope_extension(request.reference(), extension),
        Err(ErasureErrorV1::PolicyConflict)
    );
    Ok(())
}
...
#[test]
fn coordinator_corrected_submission_propagates_each_manifest_read_failure(
) -> Result<(), ErasureErrorV1> {
    for occurrence in [0, 1] {
        ...
        let rejected = coordinator.reject(original.reference(), reference(21))?;
        let correction =
            correction_for(original.reference(), rejected.state_digest(), reference(22))?;
        let corrected = corrected_request(correction.reference())?;
        let faulted = adapter.with_operation_fault(PublicCoordinatorFault {
            operation: PublicCoordinatorOperation::LoadManifest,
            occurrence,
        });
        assert_eq!(
            ErasureCoordinatorStateMachineV1::new(faulted, COORDINATOR)
                .submit_corrected(corrected, correction),
            Err(ErasureErrorV1::TrustSnapshotInvalid)
        );
    }
    ...
    let corrected = corrected_request(correction.reference())?;
    let faulted = adapter.with_operation_fault(PublicCoordinatorFault {
        operation: PublicCoordinatorOperation::Authenticate,
        occurrence: 0,
    });
    assert_eq!(
        ErasureCoordinatorStateMachineV1::new(faulted, COORDINATOR)
...
fn coordinator_persists_typed_freeze_rejection_and_rejects_wrong_provenance(
    ...
    for (authorization, expected) in [
        ...
        if result.is_ok() {
            ...
            let recovered = ErasureCoordinatorStateMachineV1::new(adapter, COORDINATOR)
                .submit(request.clone(), request.provenance())?;
            assert_eq!(recovered.lifecycle(), ErasureLifecycleV1::Rejected);
        }
    }
    Ok(())
}
...
fn coordinator_recovery_rejects_malformed_private_graph_envelopes() -> Result<(), ErasureErrorV1> {
    ...
    for field in 0..7 {
        assert_scope_graph_mutation_rejected(|graph| {
            graph.adapter.replace_scope_node_field(
                graph.request.reference(),
                0,
                field,
                Value::Bool(true),
    ...
    for field in 0..13 {
        assert_resolution_graph_mutation_rejected(|graph| {
            graph.adapter.replace_resolution_field(
                graph.request.reference(),
                0,
                field,
                Value::Bool(true),
...
fn coordinator_propagates_every_host_dependency_fault() -> Result<(), ErasureErrorV1> {
    for operation in [
        PublicCoordinatorOperation::Authenticate,
...
#[test]
fn coordinator_propagates_manifest_read_failures_from_every_recovery_entry(
) -> Result<(), ErasureErrorV1> {
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        coordinator
            .authorize(request.reference(), reference(230))
            .map(|_| ())
    })?;
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        coordinator
            .reject(request.reference(), reference(230))
            .map(|_| ())
    })?;
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        coordinator
    ...
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        coordinator
            .finalize(
                request.reference(),
                coordinator_receipt_input(target(10), 30),
            )
            .map(|_| ())
    })?;
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        let lineage_rule = reference(170);
        let scope = coordinator_scope(request.reference(), target(10), lineage_rule)?;
        coordinator
            .append_scope_extension(
                request.reference(),
                coordinator_extension(request.reference(), &scope, lineage_rule, None)?,
            )
            .map(|_| ())
    ...
    assert_manifest_read_fault_at_entry(|coordinator, request| {
        let resolution =
            ErasureAdministrativeResolutionV1::new(ErasureAdministrativeResolutionInputV1 {
                request: request.reference(),
                affected_digests: vec![reference(231)],
                action: ErasureAdministrativeResolutionActionV1::CloseContainment,
                scope_commitment: reference(232),
                policy: reference(5),
                trust: reference(6),
                principal: reference(233),
                authorization_provenance: reference(234),
                reason: reference(235),
                issue_position: 31,
                predecessor_resolution: None,
            })?;
        coordinator
            .resolve_administratively(request.reference(), &resolution)
            .map(|_| ())
    })
}
...
#[test]
fn coordinator_exact_retry_rejects_read_decode_and_inventory_failures() -> Result<(), ErasureErrorV1>
{
    let probe = complete_persisted_graph(None)?;
    let adapter = probe.adapter.with_operation_fault(PublicCoordinatorFault {
        operation: PublicCoordinatorOperation::ReadObject,
        occurrence: u64::MAX,
    });
    let observer = adapter.clone();
    ErasureCoordinatorStateMachineV1::new(adapter, COORDINATOR)
        .submit(probe.request.clone(), probe.request.provenance())?;
    let recovery_reads = observer.operation_fault_hits();

    let graph = complete_persisted_graph(None)?;
    let adapter = graph.adapter.with_operation_fault(PublicCoordinatorFault {
        operation: PublicCoordinatorOperation::ReadObject,
        occurrence: recovery_reads,
    });
    let mut coordinator = ErasureCoordinatorStateMachineV1::new(adapter, COORDINATOR);
    coordinator.submit(graph.request.clone(), graph.request.provenance())?;
    assert_eq!(
        coordinator.finalize(
            graph.request.reference(),
            coordinator_receipt_input(target(10), 30),
    ...
    graph
        .adapter
        .insert_object(graph.receipt.receipt_digest(), vec![0]);
    assert!(coordinator
        .finalize(
            graph.request.reference(),
            coordinator_receipt_input(target(10), 30),
    ...
    coordinator.submit(graph.request.clone(), graph.request.provenance())?;
    let mut incomplete = coordinator_receipt_input(target(10), 30);
    incomplete.inventories.artifacts.clear();
    assert!(coordinator
        .finalize(graph.request.reference(), incomplete)
        .is_err());
    Ok(())
}

#[test]
fn receipt_history_verifies_the_persisted_terminal_chain() -> Result<(), ErasureErrorV1> {
    let graph = complete_persisted_graph(None)?;
    let terminal = graph
        .adapter
    ...
    assert_eq!(
        graph
            .receipt
            .verify_history(&ResolverReply::State(Box::new(terminal))),
        Err(ErasureErrorV1::ProvenanceMissing)
    );
    assert_eq!(graph.receipt.verify_history(&graph.adapter), Ok(()));
    Ok(())
}
...
#[test]
fn receipt_rejects_each_independently_oversized_collection() -> Result<(), ErasureErrorV1> {
    let (input, _) = complete_receipt_input()?;

    let mut acknowledgements = input.clone();
    acknowledgements.acknowledgements.resize(
        ERASURE_MAX_ACKNOWLEDGEMENTS_PER_ATTEMPT + 1,
        acknowledgements.acknowledgements[0],
    );
    assert_eq!(
        ErasureReceiptV1::new(acknowledgements),
        Err(ErasureErrorV1::ScopeInvalid)
    );
    ...
    targets
        .frozen_targets
        .resize(ERASURE_MAX_INVENTORY_RESULTS + 1, targets.frozen_targets[0]);
    assert_eq!(
        ErasureReceiptV1::new(targets),
        Err(ErasureErrorV1::ScopeInvalid)
    );

    let mut inventory = input;
    inventory.inventories.artifacts.resize(
        ERASURE_MAX_INVENTORY_RESULTS + 1,
        inventory.inventories.artifacts[0],
    );
    assert_eq!(
        ErasureReceiptV1::new(inventory),
        Err(ErasureErrorV1::ScopeInvalid)
    );
    Ok(())
}
