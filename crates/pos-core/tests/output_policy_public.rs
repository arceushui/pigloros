...
fn canonical_encoding_and_domain_digest_match_independent_oracles() -> TestResult {
    ...
    let independent = Value::Array(vec![
        Value::Bytes(b"EOP1".to_vec()),
        Value::Integer(1.into()),
        Value::Bytes(7u128.to_be_bytes().to_vec()),
        ...
        Value::Array(vec![Value::Array(vec![
            Value::Text("world.obs.v1".to_owned()),
...
fn empty_policy_and_every_integer_width_roundtrip() -> TestResult {
    ...
    assert_eq!(
        OutputPolicyV1::from_canonical_cbor(&empty.to_canonical_cbor())?,
        empty
    );
    for number in [1, 23, 24, 255, 256, 65_535, 65_536, u32::MAX] {
        let row = OutputDeclarationV1::new(
            "view".to_owned(),
        ...
        assert_eq!(
            OutputPolicyV1::from_canonical_cbor(&policy.to_canonical_cbor())?,
            policy
        );
    }
    Ok(())
...
fn declaration_counts_and_utf8_bounds_are_exact() -> TestResult {
    for count in [1, 23, 24, 255, 256] {
        let declarations = (0..count)
            .map(|index| declaration(&format!("{index:03}{}", "x".repeat(125))))
    ...
    let over = (0..257)
        .map(|index| declaration(&format!("{index:03}")))
        .collect::<Result<Vec<_>, _>>()?;
    assert_eq!(
        OutputPolicyV1::new(input(over)),
    ...
    for version in [String::new(), "v".repeat(65)] {
        ...
        fields.plugin_version = version;
        assert_eq!(
            OutputPolicyV1::new(fields),
            Err(OutputPolicyErrorV1::FieldOutOfBounds)
        );
    }
    Ok(())
...
fn fidelity_and_authority_rules_cannot_be_defaulted() -> TestResult {
    for authority in [
        ...
        for (level, stride, group) in [
            (OutputFidelityV1::L0, None, None),
            (OutputFidelityV1::L1, Some(32), None),
            (OutputFidelityV1::L2, None, Some(10)),
        ] {
            let result =
                OutputDeclarationV1::new("output".to_owned(), authority, level, 1, stride, group);
    ...
    for (level, stride, group) in [
        (OutputFidelityV1::L0, Some(1), None),
        (OutputFidelityV1::L0, None, Some(10)),
        (OutputFidelityV1::L1, None, None),
        (OutputFidelityV1::L1, Some(0), None),
        (OutputFidelityV1::L1, Some(33), None),
        (OutputFidelityV1::L1, Some(1), Some(10)),
        (OutputFidelityV1::L2, None, None),
        (OutputFidelityV1::L2, None, Some(9)),
        (OutputFidelityV1::L2, Some(1), Some(10)),
    ] {
        assert_eq!(
            OutputDeclarationV1::new(
                "output".to_owned(),
            ...
            Err(OutputPolicyErrorV1::IncompatibleDeclaration)
        );
    }
    assert_eq!(
        OutputDeclarationV1::new(
...
fn duplicates_order_zero_references_and_revision_are_rejected() -> TestResult {
    for kinds in [["a", "a"], ["b", "a"]] {
        assert_eq!(
            OutputPolicyV1::new(input(vec![declaration(kinds[0])?, declaration(kinds[1])?])),
    ...
    for position in 0..5 {
        let mut fields = input(Vec::new());
        match position {
            0 => fields.implementation_hash = Hash::zero(),
            1 => fields.base_configuration_digest = Hash::zero(),
            2 => fields.executable_profile_hash = Hash::zero(),
            3 => fields.retention_policy_hash = Hash::zero(),
            _ => fields.policy_revision = 0,
        }
        assert_eq!(
            OutputPolicyV1::new(fields),
            Err(OutputPolicyErrorV1::FieldOutOfBounds)
...
fn every_record_field_mutation_changes_identity() -> TestResult {
    ...
    for position in 0..8 {
        ...
        match position {
            0 => changed.plugin_id = PluginId::from_ulid(ulid::Ulid::from(8u128)),
            ...
            6 => changed.policy_revision = 2,
            _ => changed.output_declarations = vec![declaration("b")?],
        }
...
fn malformed_wire_fields_reject_through_public_decoder() -> TestResult {
    ...
    for (path, replacement, error) in [
        (
            ...
            OutputPolicyErrorV1::InvalidEncoding,
        ),
        (
            vec![1],
            Value::Integer(2.into()),
            OutputPolicyErrorV1::UnsupportedValue,
        ),
        (
            vec![2],
            Value::Bytes(vec![7; 15]),
            OutputPolicyErrorV1::InvalidEncoding,
        ),
        (
            vec![3],
            Value::Text(String::new()),
            OutputPolicyErrorV1::FieldOutOfBounds,
        ),
        (
            vec![3],
            Value::Text("v".repeat(65)),
            OutputPolicyErrorV1::FieldOutOfBounds,
        ),
        (
            vec![4],
        ...
        (
            vec![9, 0, 4],
            Value::Integer(1.into()),
            OutputPolicyErrorV1::IncompatibleDeclaration,
        ),
        (
            vec![9, 0, 5],
        ...
        assert_eq!(
            OutputPolicyV1::from_canonical_cbor(&encode(&value)?),
            Err(error)
        );
    }
    Ok(())
...
fn forbidden_wire_types_and_outer_cardinalities_reject() -> TestResult {
    ...
    for replacement in [
        ...
        let mut value = base.clone();
        replace(&mut value, &[8], replacement)?;
        assert_eq!(
            OutputPolicyV1::from_canonical_cbor(&encode(&value)?),
...
fn truncation_nonpreferred_widths_and_outer_bounds_reject() -> TestResult {
    ...
    for end in 0..bytes.len() {
        assert!(OutputPolicyV1::from_canonical_cbor(&bytes[..end]).is_err());
    }
    for prefix in [
        vec![0x98, 10],
        vec![0x99, 0, 10],
        ...
        assert_eq!(
            OutputPolicyV1::from_canonical_cbor(&malformed),
            Err(OutputPolicyErrorV1::NonCanonical)
        );
    }
    ...
    trailing.resize(65_536, 0);
    assert_eq!(
        OutputPolicyV1::from_canonical_cbor(&trailing),
        Err(OutputPolicyErrorV1::NonCanonical)
    );
    trailing.push(0);
    assert_eq!(
...
fn nested_cardinalities_and_declared_allocation_bounds_reject() -> TestResult {
    ...
    for count in [5, 7] {
        ...
        let Value::Array(rows) = &mut fields[9] else {
            return Err("fixture rows".into());
        };
        let Value::Array(row) = &mut rows[0] else {
            return Err("fixture row".into());
        };
        if count == 5 {
            row.pop();
        } else {
    ...
    assert_eq!(
        OutputPolicyV1::from_canonical_cbor(&huge),
        Err(OutputPolicyErrorV1::FieldOutOfBounds)
    );
    Ok(())
}
...
fn structural_errors_have_closed_safe_diagnostics() {
    for (error, expected) in [
        (
            OutputPolicyErrorV1::InvalidEncoding,
            "invalid output policy encoding",
        ),
        (
            OutputPolicyErrorV1::UnsupportedValue,
            "unsupported output policy value",
        ),
        (
            OutputPolicyErrorV1::FieldOutOfBounds,
            "output policy field out of bounds",
        ),
        (
            OutputPolicyErrorV1::NonCanonical,
            "noncanonical output policy",
        ),
        (
            OutputPolicyErrorV1::IncompatibleDeclaration,
            "incompatible output declaration",
        ),
    ] {
