...
fn literal_empty_catalog_and_binding_pin_exact_bytes_and_digest() -> TestResult {
    ...
    assert_eq!(ManifestAdmissionCatalogV1::from_canonical_cbor(&catalog_bytes)?, catalog);
    // Independently computed with b3sum over the literal domain/NUL/CBOR bytes.
    assert_eq!(
        catalog.digest().as_bytes(),
        &[
            0x56, 0x5b, 0x6f, 0x4b, 0x67, 0xe8, 0xba, 0xfe, 0x07, 0xa3, 0xcc, 0x34, 0x52,
            0x79, 0xea, 0xa4, 0x69, 0xb1, 0x8a, 0xc9, 0xb0, 0xe0, 0x02, 0xa0, 0x6a, 0x01,
            0x67, 0xc7, 0xac, 0xa4, 0x61, 0x22,
        ]
    );
    ...
    assert_eq!(ManifestSlotBindingV1::from_canonical_cbor(&binding_bytes)?, binding);
    assert_eq!(
        binding.digest().as_bytes(),
        &[
            0x28, 0x56, 0x42, 0xee, 0x21, 0xd8, 0xcd, 0xd7, 0x31, 0xe9, 0x2d, 0xb8, 0x87,
            0x44, 0xa8, 0x7c, 0x05, 0x43, 0x30, 0x29, 0x74, 0x00, 0x3b, 0x21, 0x84, 0xa8,
            0x1f, 0xe2, 0x11, 0xf1, 0x27, 0x84,
        ]
    );
    Ok(())
}
...
fn complete_same_name_rows_roundtrip_without_key_collapse() -> TestResult {
    ...
    assert_ne!(catalog.as_input().rows[0].plugin_id, catalog.as_input().rows[1].plugin_id);
    assert_eq!(
...
fn malformed_nonpreferred_trailing_and_oversized_wire_rejects() -> TestResult {
    ...
    assert_eq!(
        ManifestAdmissionCatalogV1::from_canonical_cbor(&vec![
            0;
            MAX_MANIFEST_ADMISSION_CATALOG_BYTES_V1 + 1
        ]),
        Err(ManifestOwnerLinkErrorV1::FieldOutOfBounds)
    );
    assert_eq!(
    ...
    assert_eq!(
        ManifestSlotAdmissionReceiptV1::from_canonical_cbor(&vec![
            0;
            MAX_MANIFEST_SLOT_ADMISSION_RECEIPT_BYTES_V1 + 1
        ]),
        Err(ManifestOwnerLinkErrorV1::FieldOutOfBounds)
    );
    Ok(())
