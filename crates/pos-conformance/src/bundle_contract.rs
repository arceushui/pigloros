...
mod tests {
    ...
    #[test]
    fn independent_component_guards_reject_malformed_values(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let bundle = signed_bundle(&profile(), BundleModeV1::Local)?;
        let valid = bundle_value(&bundle);
    ...
    fn independent_member_component_rejections(
        ...
        assert_eq!(
            independent_member_paths_and_profile(members, &[Value::Null]),
            Err(BundleContractErrorV1::ArchiveEncodingInvalid)
        );
        let mut invalid_member = members[0].clone();
        let Value::Array(member_fields) = &mut invalid_member else {
            return Err("member must be an array".into());
        };
        member_fields[0] = Value::Null;
        assert_eq!(
            independent_member_paths_and_profile(&[invalid_member], descriptors),
            Err(BundleContractErrorV1::ArchiveEncodingInvalid)
        );

        let mut invalid_member_bytes = members[0].clone();
        let Value::Array(member_fields) = &mut invalid_member_bytes else {
            return Err("member must be an array".into());
        };
        member_fields[1] = Value::Null;
        assert_eq!(
            independent_member_paths_and_profile(&[invalid_member_bytes], descriptors),
            Err(BundleContractErrorV1::ArchiveEncodingInvalid)
        );

        let mut invalid_member_role = members[0].clone();
        let Value::Array(member_fields) = &mut invalid_member_role else {
            return Err("member must be an array".into());
        };
        member_fields[2] = Value::Null;
        assert_eq!(
            independent_member_paths_and_profile(&[invalid_member_role], descriptors),
            Err(BundleContractErrorV1::ArchiveEncodingInvalid)
        );

        assert_eq!(
            independent_member_paths_and_profile(
                &[members[0].clone(), members[0].clone()],
                &[descriptors[0].clone(), descriptors[0].clone()],
            ),
            Err(BundleContractErrorV1::MemberDigestMismatch)
        );
        let mut wrong_descriptor = descriptors[0].clone();
        let Value::Array(descriptor_fields) = &mut wrong_descriptor else {
            return Err("descriptor must be an array".into());
        };
        descriptor_fields[0] = Value::Text("wrong/path".to_owned());
        assert_eq!(
    ...
    fn independent_expected_result_component_rejections(
        ...
        assert_eq!(
            independent_verify_expected_results(
                &[expected_results[0].clone(), expected_results[0].clone()],
                descriptors,
                &expected_paths,
            ),
            Err(BundleContractErrorV1::ExpectedResultMismatch)
        );
        let mut missing_expected = expected_results[0].clone();
        let Value::Array(expected_fields) = &mut missing_expected else {
            return Err("expected result must be an array".into());
        };
        expected_fields[4] = Value::Text("expected/missing".to_owned());
        assert_eq!(
            independent_verify_expected_results(&[missing_expected], descriptors, &expected_paths),
            Err(BundleContractErrorV1::MemberMissing)
        );
        let mut wrong_expected_digest = expected_results[0].clone();
        let Value::Array(expected_fields) = &mut wrong_expected_digest else {
            return Err("expected result must be an array".into());
        };
        expected_fields[5] = Value::Bytes(vec![9; 32]);
        assert_eq!(
            independent_verify_expected_results(
                &[wrong_expected_digest],
                descriptors,
    ...
    fn independent_profile_component_rejections(
        ...
        let profile_value: Value = ciborium::from_reader(Cursor::new(profile_bytes))?;
        let mut wrong_profile_shape = profile_value.clone();
        let Value::Array(profile_fields) = &mut wrong_profile_shape else {
            return Err("profile must be an array".into());
        };
        profile_fields.pop();
        let wrong_profile_bytes = encode_archive_value(&wrong_profile_shape)?;
        assert_eq!(
        ...
        profile_fields[0] = Value::Text("CPF0".to_owned());
        let wrong_profile_bytes = encode_archive_value(&wrong_profile_magic)?;
        assert_eq!(
            independent_verify_profile(&wrong_profile_bytes, lifecycle, &manifest[3]),
            Err(BundleContractErrorV1::ProfileInvalid)
        );
