...
fn complete_command(directory: &Path) -> TestResult<Command> {
    ...
    command_for_corpus(directory, &corpus)
}

fn command_for_corpus(directory: &Path, corpus: &support::Corpus) -> TestResult<Command> {
...
#[cfg(unix)]
#[test]
fn command_emits_a_self_verified_report_through_the_root_selector_boundary() -> TestResult {
    verify_staged_corpus()?;
    let directory = tempfile::tempdir()?;
    let output = complete_command(directory.path())?.output()?;
    assert!(
...
fn command_accepts_an_exact_authenticated_profile_byte_cap() -> TestResult {
    ...
    let output = command_for_corpus(directory.path(), &corpus)?.output()?;
    assert!(
        output.status.success(),
        "evaluator stderr: {}",
        String::from_utf8_lossy(&output.stderr)
...
fn command_binds_the_loaded_executable_after_its_path_is_replaced() -> TestResult {
    ...
    std::os::unix::fs::symlink(&loaded_executable, &executable)?;
    ...
    // the launcher path is replaced. The launcher is a symlink so the rename
    // does not attempt to replace a live executable inode.
...
#[cfg(unix)]
#[test]
fn command_closes_input_selector_and_evaluation_failures() -> TestResult {
    for input in [
        "request.cbor",
        "bundle.cfb1",
        "policy.tps1",
    ...
    let directory = tempfile::tempdir()?;
    let unavailable = complete_command(directory.path())?.output()?;
    assert!(
...
fn command_rejects_duplicate_and_unsorted_public_identity_options() -> TestResult {
    ...
    for legacy in [
        ["--adapter", "/bin/false"],
        ["--adapter-arg", "legacy-argument"],
    ] {
        let output = complete_command(directory.path())?.args(legacy).output()?;
        assert!(!output.status.success());
    }
