    ...
    impl<T, E: std::fmt::Debug> TestValueExt<T> for Result<T, E> {
        fn test_ok(self) -> T {
            self.unwrap_or_else(|error| {
                eprintln!("unexpected moat proof test error: {error:?}");
...
mod coverage_entrypoints {
    ...
    fn test_ok<T, E: std::fmt::Debug>(result: Result<T, E>) -> T {
        result.unwrap_or_else(|error| {
            eprintln!("unexpected moat proof coverage error: {error:?}");
...
mod run_coverage_entrypoints {
    ...
    fn test_ok<T, E: std::fmt::Debug>(result: Result<T, E>) -> T {
        result.unwrap_or_else(|error| {
            eprintln!("unexpected moat proof run coverage error: {error:?}");
