...
- **Timeline bound:** one Gateway process accepts at most 64 **root** Timelines. Forks and identity-preserving imported children do not consume root capacity. Quota checks use a bounded scalar store seam that stops at quota + 1 and never lists or clones Timeline metadata. The memory adapter maintains exact root cardinality with Timeline mutations, so checks are O(1) regardless of Fork count. SQLite seeks the covering `(parent_id, id)` index and reads at most 65 root IDs without visiting child rows.
